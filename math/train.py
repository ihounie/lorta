...
import datasets
import numpy as np
from peft import LorTaConfig, LoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # Lorta setting
    use_lorta : Optional[bool] = field(default=False,metadata={"help": "Use Lorta instead of LoRA."},)
...
def build_model(script_args, checkpoint_dir):
    ...
    if not script_args.full_finetune:
        ...
        if checkpoint_dir is not None:
            ...
            if script_args.use_lorta:
                logger.info(f'Init Lorta modules...')
